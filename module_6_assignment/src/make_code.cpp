...
private:
    ...
    void update_waypoints(){
        ...
        if (imu_msg && ekf_msg) {
            geometry_msgs::msg::Point gps_imu_;

            gps_imu_.x = gps_msg.x;
            gps_imu_.y = gps_msg.y;
            gps_imu_.z = 0.0;
            ...
            // Adding point if its new
            if (!gps_imu_path_.empty() && !ekf_path_.empty() &&
                (gps_imu_path_.back().x != gps_imu_.x  || gps_imu_path_.back().y != gps_imu_.y || gps_imu_path_.back().z != gps_imu_.z) &&
                (ekf_path_.back().x != ekf_msg->x  || ekf_path_.back().y != ekf_msg->y || ekf_path_.back().z != ekf_msg->z)) {
                gps_imu_path_.push_back(gps_imu_);
                ekf_path_.push_back(*ekf_msg);

                RCLCPP_INFO(this->get_logger(), "Points has been added");
            }
    ...
    void publishMarkers() {
        ...
        RCLCPP_INFO(this->get_logger(), "Published %zu goals ",
                    waypoints_.size());
    }
